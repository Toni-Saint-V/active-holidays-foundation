...
const landingCopyByProduct: Record<ProductType, LandingCopy> = {
  ...
  residency_es: {
    ...
    ai: {
      summary: "AI: сначала зафиксируй легальность дохода — это ключ к рабочему пути ВНЖ",
      reasons: [
        "Для ВНЖ слабое подтверждение дохода ломает маршрут даже при полном досье документов.",
        "Экран сразу отделяет жизнеспособный путь от опций, которые выглядят красиво, но не проходят риск-проверку."
      ],
      action: "Проверь рабочий путь, собери первый обязательный блок досье и зафиксируй ключевой риск."
    },
  ...
  insurance_adult: {
    ...
    ai: {
      ...
      reasons: [
        ...
        "Сначала валидируем пригодность и документы, затем даём переход к покупке."
      ],

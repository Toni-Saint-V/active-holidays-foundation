...
function aiInsight(
  ...
  if (missingDocs.length > 0) {
    ...
    return {
      summary: `AI: сначала закрой документ «${primaryGap.label}», иначе маршрут и следующий шаг остаются рискованными`,
      reasons: [
        `Документный контур сейчас: ${documentsCountLabel(missingDocs.length)} без подтверждения.`,
        criticalRiskDetail
          ? `Проверь и зафиксируй связанный риск: ${criticalRiskDetail}`
          : "Проверь обязательный пакет документов и только потом открывай переход по маршруту."
      ],
      action:
        "Следующий шаг: открой документы, закрой первый блокер и подтверди пакет перед переходом к действию."
    };
  }
  ...
  if (recommendedScenario) {
    if (
      ...
      return {
        summary: `AI: сценарий «${recommendedScenario.title}» блокирует маршрут и не готов к авто-переходу`,
        reasons: [
          `Причина остановки: ${
          ...
          "Проверь риск и источники вручную: этот сценарий нужен для диагностики, а не для мгновенного запуска."
        ],
        action: "Следующий шаг: передай сценарий менеджеру, проверь источники и зафиксируй безопасный маршрут вручную."
      };
    }

    if (recommendedScenario.safetyStatus === "degraded_usable") {
      return {
        summary: `AI: «${recommendedScenario.title}» — допустимый fallback по маршруту, но не лучший путь`,
        reasons: [
          "Сценарий рабочий, но по качеству уступает текущему маршруту по ключевым условиям и рискам.",
          "Проверь документные различия и держи этот путь как запасной, а не как мгновенную замену."
        ],
        action:
          "Следующий шаг: сравни сценарии по документам и рискам, затем зафиксируй решение о переключении."
      };
    }

    return {
      summary: `AI: сценарий «${recommendedScenario.title}» может усилить текущий маршрут без потери контроля риска`,
      reasons: [
        "Базовый маршрут уже рабочий: сценарий нужен для усиления качества, а не для спасения кейса.",
        "Проверь риски, документы и следующее действие перед любым переключением."
      ],
      action:
        "Следующий шаг: открой сравнение и подтверди, что усиление не увеличивает операционные риски."
    };
  }
  ...
  return {
    ...
    reasons: [
      "Ключевые сигналы по маршруту уже согласованы и путь не требует экстренного пересмотра.",
      "Проверь выполнение следующего действия по документам и зафиксируй результат без отклонений."
    ],
